...
;GPS_Parser.c,110 :: 		void swap_raw_data()
;GPS_Parser.c,114 :: 		lat_second_s[0] = GPGGA_Data[13];
	MOVF        _GPGGA_Data+13, 0 
	MOVWF       _lat_second_s+0 
;GPS_Parser.c,115 :: 		lat_second_s[1] = GPGGA_Data[14];
	MOVF        _GPGGA_Data+14, 0 
	MOVWF       _lat_second_s+1 
;GPS_Parser.c,116 :: 		lat_second_s[2] = GPGGA_Data[16];
	MOVF        _GPGGA_Data+16, 0 
	MOVWF       _lat_second_s+2 
;GPS_Parser.c,117 :: 		lat_second_s[3] = GPGGA_Data[17];
	MOVF        _GPGGA_Data+17, 0 
	MOVWF       _lat_second_s+3 
;GPS_Parser.c,118 :: 		lat_second_s[4] = GPGGA_Data[18];
	MOVF        _GPGGA_Data+18, 0 
	MOVWF       _lat_second_s+4 
;GPS_Parser.c,119 :: 		lat_second_s[5] = GPGGA_Data[19];
	MOVF        _GPGGA_Data+19, 0 
	MOVWF       _lat_second_s+5 
;GPS_Parser.c,120 :: 		lat_second_s[6] = GPGGA_Data[20];
	MOVF        _GPGGA_Data+20, 0 
	MOVWF       _lat_second_s+6 
;GPS_Parser.c,122 :: 		lat_first[2] = GPGGA_Data[11];
...
;GPS_Parser.c,126 :: 		lat_second_i = ((lat_second_s[0] - '0') * 1000) + ((lat_second_s[1] - '0') * 100) + ((lat_second_s[2] - '0') * 10) + (lat_second_s[3] - '0');
	...
	MOVF        R1, 0 
	ADDWFC      _lat_second_i+1, 1 
;GPS_Parser.c,128 :: 		long_first[3] = GPGGA_Data[24];
	MOVF        _GPGGA_Data+24, 0 
	MOVWF       _long_first+3 
;GPS_Parser.c,129 :: 		long_first[2] = GPGGA_Data[25];
...
;GPS_Parser.c,133 :: 		long_second_s[1] = GPGGA_Data[28];
	MOVF        _GPGGA_Data+28, 0 
	MOVWF       _long_second_s+1 
;GPS_Parser.c,134 :: 		long_second_s[2] = GPGGA_Data[30];
	MOVF        _GPGGA_Data+30, 0 
	MOVWF       _long_second_s+2 
;GPS_Parser.c,135 :: 		long_second_s[3] = GPGGA_Data[31];
	MOVF        _GPGGA_Data+31, 0 
	MOVWF       _long_second_s+3 
;GPS_Parser.c,136 :: 		long_second_s[4] = GPGGA_Data[32];
	MOVF        _GPGGA_Data+32, 0 
	MOVWF       _long_second_s+4 
;GPS_Parser.c,137 :: 		long_second_s[5] = GPGGA_Data[33];
	MOVF        _GPGGA_Data+33, 0 
	MOVWF       _long_second_s+5 
;GPS_Parser.c,138 :: 		long_second_s[6] = GPGGA_Data[34];
	MOVF        _GPGGA_Data+34, 0 
	MOVWF       _long_second_s+6 
;GPS_Parser.c,140 :: 		long_second_i = ((long_second_s[0] - '0') * 1000) + ((long_second_s[1] - '0') * 100) + ((long_second_s[2] - '0') * 10) + (long_second_s[3] - '0');
	MOVLW       48
...
;GPS_Parser.c,144 :: 		void GPS_GetData(){
;GPS_Parser.c,145 :: 		unsigned char Temp = 0, Index = 0, rIndex = 9;
	CLRF        GPS_GetData_Temp_L0+0 
	CLRF        GPS_GetData_Index_L0+0 
;GPS_Parser.c,147 :: 		while (!UART1_Data_Ready());
L_GPS_GetData21:
...
L_GPS_GetData22:
;GPS_Parser.c,148 :: 		Temp = UART1_Read();
	CALL        _UART1_Read+0, 0
	MOVF        R0, 0 
	MOVWF       GPS_GetData_Temp_L0+0 
;GPS_Parser.c,149 :: 		if (Temp == '$') {
	MOVF        R0, 0 
	XORLW       36
	BTFSS       STATUS+0, 2 
	GOTO        L_GPS_GetData23
;GPS_Parser.c,150 :: 		while (!UART1_Data_Ready());
L_GPS_GetData24:
	CALL        _UART1_Data_Ready+0, 0
	MOVF        R0, 1 
	BTFSS       STATUS+0, 2 
...
L_GPS_GetData25:
;GPS_Parser.c,151 :: 		Temp = UART1_Read();
	CALL        _UART1_Read+0, 0
	MOVF        R0, 0 
	MOVWF       GPS_GetData_Temp_L0+0 
;GPS_Parser.c,152 :: 		if (Temp == 'G') {
	MOVF        R0, 0 
	XORLW       71
	BTFSS       STATUS+0, 2 
	GOTO        L_GPS_GetData26
;GPS_Parser.c,153 :: 		while (!UART1_Data_Ready());
L_GPS_GetData27:
	CALL        _UART1_Data_Ready+0, 0
	MOVF        R0, 1 
	BTFSS       STATUS+0, 2 
...
L_GPS_GetData28:
;GPS_Parser.c,154 :: 		Temp = UART1_Read();
	CALL        _UART1_Read+0, 0
	MOVF        R0, 0 
	MOVWF       GPS_GetData_Temp_L0+0 
;GPS_Parser.c,155 :: 		if (Temp == 'P') {
	MOVF        R0, 0 
	XORLW       80
	BTFSS       STATUS+0, 2 
	GOTO        L_GPS_GetData29
;GPS_Parser.c,156 :: 		while (!UART1_Data_Ready());
L_GPS_GetData30:
	CALL        _UART1_Data_Ready+0, 0
	MOVF        R0, 1 
	BTFSS       STATUS+0, 2 
...
L_GPS_GetData31:
;GPS_Parser.c,157 :: 		Temp = UART1_Read();
	CALL        _UART1_Read+0, 0
	MOVF        R0, 0 
	MOVWF       GPS_GetData_Temp_L0+0 
;GPS_Parser.c,158 :: 		if (Temp == 'G') {
	MOVF        R0, 0 
	XORLW       71
	BTFSS       STATUS+0, 2 
	GOTO        L_GPS_GetData32
;GPS_Parser.c,159 :: 		while (!UART1_Data_Ready());
L_GPS_GetData33:
	CALL        _UART1_Data_Ready+0, 0
	MOVF        R0, 1 
	BTFSS       STATUS+0, 2 
...
L_GPS_GetData34:
;GPS_Parser.c,160 :: 		Temp = UART1_Read();
	CALL        _UART1_Read+0, 0
	MOVF        R0, 0 
	MOVWF       GPS_GetData_Temp_L0+0 
;GPS_Parser.c,161 :: 		if (Temp == 'G') {
	MOVF        R0, 0 
	XORLW       71
	BTFSS       STATUS+0, 2 
	GOTO        L_GPS_GetData35
;GPS_Parser.c,162 :: 		while (!UART1_Data_Ready());
L_GPS_GetData36:
	CALL        _UART1_Data_Ready+0, 0
	MOVF        R0, 1 
	BTFSS       STATUS+0, 2 
...
L_GPS_GetData37:
;GPS_Parser.c,163 :: 		Temp = UART1_Read();
	CALL        _UART1_Read+0, 0
	MOVF        R0, 0 
	MOVWF       GPS_GetData_Temp_L0+0 
;GPS_Parser.c,164 :: 		if (Temp == 'A') {
	MOVF        R0, 0 
	XORLW       65
	BTFSS       STATUS+0, 2 
	GOTO        L_GPS_GetData38
;GPS_Parser.c,165 :: 		while (!UART1_Data_Ready());
L_GPS_GetData39:
	CALL        _UART1_Data_Ready+0, 0
	MOVF        R0, 1 
	BTFSS       STATUS+0, 2 
...
L_GPS_GetData40:
;GPS_Parser.c,166 :: 		Temp = UART1_Read(); //
	CALL        _UART1_Read+0, 0
	MOVF        R0, 0 
	MOVWF       GPS_GetData_Temp_L0+0 
;GPS_Parser.c,167 :: 		while (Temp != '\n') {
L_GPS_GetData41:
	MOVF        GPS_GetData_Temp_L0+0, 0 
	XORLW       10
	BTFSC       STATUS+0, 2 
	GOTO        L_GPS_GetData42
;GPS_Parser.c,168 :: 		GPGGA_Data[Index] = Temp;
	MOVLW       _GPGGA_Data+0
	MOVWF       FSR1L+0 
	...
	MOVF        GPS_GetData_Temp_L0+0, 0 
	MOVWF       POSTINC1+0 
;GPS_Parser.c,170 :: 		while (!UART1_Data_Ready());
L_GPS_GetData43:
	CALL        _UART1_Data_Ready+0, 0
	MOVF        R0, 1 
	BTFSS       STATUS+0, 2 
...
L_GPS_GetData44:
;GPS_Parser.c,171 :: 		Temp = UART1_Read();
	CALL        _UART1_Read+0, 0
	MOVF        R0, 0 
	MOVWF       GPS_GetData_Temp_L0+0 
;GPS_Parser.c,172 :: 		Index++;
	INCF        GPS_GetData_Index_L0+0, 1 
;GPS_Parser.c,174 :: 		}
	GOTO        L_GPS_GetData41
L_GPS_GetData42:
;GPS_Parser.c,175 :: 		Index = 0;
	CLRF        GPS_GetData_Index_L0+0 
;GPS_Parser.c,176 :: 		cooldown++;
	INCF        _cooldown+0, 1 
;GPS_Parser.c,177 :: 		swap_raw_data();
	CALL        _swap_raw_data+0, 0
;GPS_Parser.c,179 :: 		sprinti(clk, "%8u", lat_second_i);
	...
	CALL        _sprinti+0, 0
;GPS_Parser.c,181 :: 		lat_second_f = (lat_second_i + 0.25) / 0.6;
	MOVF        _lat_second_i+0, 0 
	MOVWF       R0 
	MOVF        _lat_second_i+1, 0 
	MOVWF       R1 
...
;GPS_Parser.c,182 :: 		sprintf(lat_second_s, "%8f", lat_second_f);
	...
	CALL        _sprintf+0, 0
;GPS_Parser.c,184 :: 		sprinti(clk2, "%8u", long_second_i);
...
;GPS_Parser.c,186 :: 		if(long_second_f < 1000)
	MOVLW       0
	MOVWF       R4 
	MOVLW       0
	MOVWF       R5 
...
;GPS_Parser.c,188 :: 		sprintf(long_second_s2, "%8f", long_second_f);
	...
	MOVWF       FARG_sprintf_wh+8 
	CALL        _sprintf+0, 0
;GPS_Parser.c,189 :: 		long_second_s[1] = long_second_s2[0];
	MOVF        _long_second_s2+0, 0 
	MOVWF       _long_second_s+1 
;GPS_Parser.c,190 :: 		long_second_s[2] = long_second_s2[1];
	MOVF        _long_second_s2+1, 0 
	MOVWF       _long_second_s+2 
;GPS_Parser.c,191 :: 		long_second_s[3] = long_second_s2[2];
	MOVF        _long_second_s2+2, 0 
	MOVWF       _long_second_s+3 
;GPS_Parser.c,192 :: 		long_second_s[4] = long_second_s2[3];
	MOVF        _long_second_s2+3, 0 
	MOVWF       _long_second_s+4 
;GPS_Parser.c,193 :: 		long_second_s[5] = long_second_s2[4];
	MOVF        _long_second_s2+4, 0 
	MOVWF       _long_second_s+5 
;GPS_Parser.c,194 :: 		long_second_s[6] = long_second_s2[5];
	MOVF        _long_second_s2+5, 0 
	MOVWF       _long_second_s+6 
;GPS_Parser.c,195 :: 		long_second_s[0] = '0';
	MOVLW       48
	MOVWF       _long_second_s+0 
;GPS_Parser.c,196 :: 		}else
	GOTO        L_GPS_GetData46
L_GPS_GetData45:
;GPS_Parser.c,198 :: 		sprintf(long_second_s, "%8f", long_second_f);
...
;GPS_Parser.c,215 :: 		void Usend(char* cmd)
;GPS_Parser.c,217 :: 		UART1_Write_Text(cmd);
...
;GPS_Parser.c,222 :: 		void init_pins()
;GPS_Parser.c,224 :: 		TRISB.B7 = 1; // input
	BSF         TRISB+0, 7 
;GPS_Parser.c,225 :: 		TRISB.B6 = 1; // input
	BSF         TRISB+0, 6 
;GPS_Parser.c,226 :: 		TRISB.B5 = 1; // input
	BSF         TRISB+0, 5 
...
;GPS_Parser.c,231 :: 		Send_Ready = 0;
	CLRF        _Send_Ready+0 
;GPS_Parser.c,232 :: 		Auto = 0;
	CLRF        _Auto+0 
;GPS_Parser.c,233 :: 		Normal = 0;
	CLRF        _Normal+0 
;GPS_Parser.c,234 :: 		}
L_end_init_pins:
	RETURN      0
...
;GPS_Parser.c,364 :: 		if(Normal == 1)
	...
	BTFSS       STATUS+0, 2 
	GOTO        L_main79
;GPS_Parser.c,366 :: 		PORTD.B0 = 1;
	BSF         PORTD+0, 0 
;GPS_Parser.c,367 :: 		Confirm = 0;
	CLRF        _Confirm+0 
;GPS_Parser.c,368 :: 		vibrate(); // check if false trigger
	CALL        _vibrate+0, 0
;GPS_Parser.c,369 :: 		if(Confirm == 0)
	MOVF        _Confirm+0, 0 
	XORLW       0
	BTFSS       STATUS+0, 2 
	GOTO        L_main80
;GPS_Parser.c,371 :: 		while(gps_taken == 0)
L_main81:
	MOVF        _gps_taken+0, 0 
	XORLW       0
	BTFSS       STATUS+0, 2 
	GOTO        L_main82
;GPS_Parser.c,373 :: 		GPS_GetData();
	CALL        _GPS_GetData+0, 0
;GPS_Parser.c,374 :: 		}
	GOTO        L_main81
L_main82:
;GPS_Parser.c,375 :: 		cooldown = 0;
	CLRF        _cooldown+0 
